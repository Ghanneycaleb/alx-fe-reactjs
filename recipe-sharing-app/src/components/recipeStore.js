...
export const useRecipeStore = create((set, get) => ({
  recipes: [
    ...
    },
    {
      id: 3,
      title: "Caesar Salad",
      description: "Fresh crispy romaine lettuce with creamy Caesar dressing and croutons.",
      ingredients: ["2 heads romaine lettuce", "1/2 cup Parmesan cheese", "1 cup croutons", "Caesar dressing", "Anchovies", "Lemon juice"],
    ...
    },
    {
      ...
      ingredients: ["2 lbs beef chuck", "4 carrots", "3 potatoes", "2 onions", "beef broth", "tomato paste", "herbs"],
      instructions: "1. Brown beef in pot\n2. Add vegetables\n3. Pour in broth\n4. Simmer for 2 hours\n5. Season and serve",
  ...
  getFilteredRecipes: () => {
    ...
    if (state.selectedDifficulty !== 'All') {
      filtered = filtered.filter(recipe => 
        recipe.difficulty === state.selectedDifficulty
      );
    }
  ...
  getDifficulties: () => {
    const state = get();
    const difficulties = [...new Set(state.recipes.map(recipe => recipe.difficulty))];
